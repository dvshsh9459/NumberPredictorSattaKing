...
plt.title(PREDICTOR_FOR_DSWR + ' Over Time')
plt.show()
...
plt.plot(dswr_series, label='Historical ' + PREDICTOR_FOR_DSWR)
plt.plot(forecast, label='Forecast' + PREDICTOR_FOR_DSWR, color='red')
plt.legend()
plt.show()

# Print the forecasted values
print('Forecasted', PREDICTOR_FOR_DSWR, 'values for the next periods:')
print(forecast)
# Correct way to access the first value by position
print(f"Forecasted {PREDICTOR_FOR_DSWR} for the next day: {forecast.iloc[0]}")