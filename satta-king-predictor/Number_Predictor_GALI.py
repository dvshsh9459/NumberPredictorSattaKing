...
dswr_series = df[PREDICTOR_FOR_GALI]

# Step 5: Plot the historical data
plt.plot(dswr_series)
plt.title(PREDICTOR_FOR_GALI + ' Over Time')
plt.show()
...
plt.plot(dswr_series, label='Historical ' + PREDICTOR_FOR_GALI)
plt.plot(forecast, label='Forecast' + PREDICTOR_FOR_GALI, color='red')
plt.legend()
plt.show()

# Print the forecasted values
print('Forecasted', PREDICTOR_FOR_GALI, 'values for the next periods:')
print(forecast)
# Correct way to access the first value by position
print(f"Forecasted {PREDICTOR_FOR_GALI} for the next day: {forecast.iloc[0]}")